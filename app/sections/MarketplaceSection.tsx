...
const MarketplaceSection: React.FC = () => {
  ...
  const handleAddReview = () => {
    if (selectedPartner && newReview.trim() && rating > 0) {
      const updatedReviews = [
        ...selectedPartner.reviews,
        {
          id: Date.now(),
          reviewer: "Анонім",
          comment: newReview,
  ...
  const handleLikeReview = (reviewId: number) => {
    if (selectedPartner) {
      const updatedReviews = selectedPartner.reviews.map((review) =>
        review.id === reviewId ? { ...review, likes: review.likes + 1 } : review
      );
      setSelectedPartner({ ...selectedPartner, reviews: updatedReviews });
    }
  };
  ...
  return (
    <section className="p-6 max-w-5xl mx-auto bg-gradient-to-r from-green-50 to-blue-50 rounded-lg shadow-xl mt-[60px] xl:mt-[100px]">
      ...
      {selectedPartner && (
        <div className="p-6 bg-white rounded-lg shadow-lg">
          ...
          <p className="text-green-600 mt-2 font-semibold">
            Рейтинг довіри: {selectedPartner.trustScore}
          </p>

          <div className="mt-6">
            <h4 className="text-xl font-semibold text-blue-700">Відгуки:</h4>
            <div className="space-y-4">
              {selectedPartner.reviews.map((review) => (
                <div
                  key={review.id}
